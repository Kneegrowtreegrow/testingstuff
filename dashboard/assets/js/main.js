...
if (typeof $ !== 'undefined') {
              ...
              templates: {
                ...
                suggestion: function ({ name, src, subtitle, url }) {
                  return (
                    ...
                    '<img style="width:50px;height:50px;margin-right:15px" src="' +
                    src +
                    '" alt="' +
                    name +
                    '" height="32">' +
  ...
  // --------------------------------------------------------------------
  var swiperWithPagination = document.querySelector('#sidebar-swiper-with-pagination-cards');
...
// Redirect to pricing function
function redirectToPricing() {
  location.href = "https://overdogbets.com/pricing";
}
...
function getUserData() {
  ...
  fetch("https://cdn.overdogbets.com/my-data", requestOptions)
    ...
    .then((result) => {
      // Once we have user data, handle the "Commoner" plan check here
      if (result[0].plan === "Commoner") {
        // Redirect to pricing if the user is a Commoner
        redirectToPricing();
        return; // Stop further execution
      } 
      
      // User has a valid plan, process the data
      processUserData(result);

      // Dispatch events after processing data
      document.dispatchEvent(new Event('userDataLoaded'));
      document.dispatchEvent(new Event('sbAPILoaded'));
    })
    .catch((error) => {
      console.log(error);
      redirectToLogin(); // Redirect to login in case of failure
    });
}
...
function processUserData(data) {
  try {
    ...
    console.log(error);
    redirectToLogin(); // Redirect to login in case of any processing error
  }
}
